# Hotel Management — LLD Machine Coding (Python)
...
> A parallel Java implementation lives in `../java`. The class/module structure is intentionally 1:1.
...
- Book a specific room for `[check_in, check_out)` and compute `nights * rate`
...
classDiagram
    ...
    class Room {
      ...
      +book(check_in, check_out, pricing) Reservation
    }
    class RoomType {
      <<Enum>>
      STANDARD
    ...
    class HotelManagementService {
      +search_available_rooms(type, start, end)
      +book_room(room_id, start, end)
    ...
    class PricingStrategy {
      <<abstract>>
      +calculate_price(room, start, end) int
    }
...
sequenceDiagram
    ...
    C->>S: book_room(room_id, check_in, check_out)
    S->>R: book(check_in, check_out, pricing)
    R->>R: lock room reservations
    R->>R: reject if any live reservation overlaps
    R->>P: calculate_price(room, check_in, check_out)
    P-->>R: nights * rate
...
| Decision | Reasoning |
| --- | --- |
| **Half-open date ranges `[check_in, check_out)`** | Checkout day is free; adjacent bookings are naturally allowed. |
| **Overlap rule: `start1 < end2 AND start2 < end1`** | Standard interval-overlap predicate; tests cover adjacent vs true overlap. |
| **Per-room `threading.Lock` around reservation list** | The availability check and append are atomic for one room, preventing double-booking while allowing different rooms to book in parallel. |
...
```
main -> HotelManagementService.search_available_rooms
     -> Room.is_available -> date-overlap check under room lock
main -> HotelManagementService.book_room
     -> Room.book -> lock -> overlap check -> PricingStrategy.calculate_price -> Reservation
main -> check_in/check_out/cancel -> Reservation state transition
```

Module layout:
```
hotel/
├── models.py       Hotel, Room, Reservation, RoomType, ReservationStatus
├── strategies.py   PricingStrategy + NightlyPricingStrategy
├── service.py      HotelManagementService
├── exceptions.py   RoomUnavailable/NotFound and reservation state exceptions
