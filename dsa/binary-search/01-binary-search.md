...
## Problem
Given a sorted integer array `nums` in ascending order and an integer `target`, return the index of `target`. If it is not present, return `-1`. Constraints: `1 <= nums.length <= 10^4`, `-10^4 <= nums[i], target <= 10^4`, and all values in `nums` are unique.
